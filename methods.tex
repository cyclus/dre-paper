...
consumer, $j$, and producer, $i$, and the proposed resource transfer from
...
\secref{abm:dre:info}. A cost translation function, $f$, is defined that
operates on the commodity preference function to produce an appropriate cost for
the NFCTP.
...
request portfolio, $q_r$, after having sorted all arcs. The constraining values
...
A rule's condition may depend on any factor that is query-able during the
