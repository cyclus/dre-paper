...
methodolgy's progression with respect to the simulation architecture is
...
A supplier may have its production constrained in the same manner as consumers,
...
Exchage Graph have a one-to-one mapping with simulation entities. For example,
nodes in the graph represent distinct bids and requsts provided by agents.
...
is comprised of $r_N$ nodes. For notational simplicity, nodes within portfolios
are referred to with single indicies (e.g., $i$ or $j$), and collections of arcs
...
\begin{figure}
  \begin{center}
    ...
    \caption{The same exchange shown in Figure \ref{fig:ex_false} arcs colored
...
%% the two unique properties associated with the formulation. Mutual requests
%% are accounted for by removing a portfolio from the exchange after a mutual
%% bid or request is utilized. Exclusivity is accounted for through initial
%% screening -- supply-request pairs that will not match are removed from the
%% exchange \textit{a priori} -- and the use of the maximum value function.
